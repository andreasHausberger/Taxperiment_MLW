...
// output is a header row with field names and a set of rows with all the events present
    ...
    $opentype = 0;
    $openevt = array(0 => "mouseover", 1 => "click");
    $procvars = array();
        ...
        if ($procdata != "") {
            if (strpos($procdata, "<?xml") !== false) {
                ...
                for ($evt = 0; $evt < $eventnum; $evt++) {
                    ...
                    if ($evtarr[0] == 'events') {
                        $opentype = substr($evtarr[2], 0, 1);
                    }
                    if ($evtarr[0] == $openevt[$opentype]) {
                        $procvars = array_merge($procvars, array($evtarr[1] => 0));
                    }
                }
...
// get first experiment data
    ...
    $headerString = "SHOW COLUMNS FROM mlweb";
    $headerResult = $connection->query($headerString);
...
if (isset($_POST['act'])) { // start action part (caused by a submitted form)
    ...
    if ($act == "download") // action is download
    {
        ...
        for ($expcount = 0; $expcount < count($arr); $expcount++) // make datafile for each experiment
        {
            $outstr = "";
    ...
    if ($act == "process") // action is process
    {
        $i = 0;
        $opentype = 0;
        $openevt = array(0 => "mouseover", 1 => "click");
        $closetype = 0;
        $closeevt = array(0 => "mouseout", 1 => "click", 2 => "click", 3 => "-");
        ...
        for ($expcount = 0; $expcount < count($arr); $expcount++) // make datafile for each experiment
        {
            ...
            for ($j = 0; $j < count($rowarr); $j++) {
                ...
                for ($c = 1; $c < count($temprow2); $c++) {
                    if ($temprow2[$c][$eventKey] == $closeevt[$closetype]) // a close event!
                    {
                        $boxname = $temprow2[$c][$eventKey + 1];
                        if ($glprocvars[$boxname] == 0) {
                            if ($opentype == 1 & $closetype > 0) {//generate open event because this click was from an open click
                                $boxname = $temprow2[$c][$eventKey + 1];
                    ...
                    } else if ($temprow2[$c][$eventKey] == $openevt[$opentype]) // an open event!
                    {
                        $boxname = $temprow2[$c][$eventKey + 1];
                        ...
                        $localcount++;
                        $glproccount[$boxname] = $localcount;
                        if ($glprocvars[$boxname] == 0) // generate a click event without time if there was no time registered
                        {
                            $outrowtmp[$glproccount[$boxname]] = array_merge(array_slice($temprow2[$c], 0, $timeKey - 2), array($roword, $colord, $evttype, $boxname, $glprocvars[$boxname], 0));
                            $glprocvars[$boxname] = 0;
                            $glproccount[$boxname] = 0;
            ...
            for ($z = count($outrow) - 1; $z > 0; $z--) {
                if ($startCounter) {
                    $maxcount = $outrow[$z][$counterKey];
                }
                $outrow[$z][] = $outrow[$z][$counterKey] / $maxcount;
                $outrow[$z][] = $maxcount;
                if ($outrow[$z][$counterKey] == 1) {
                    $startCounter = true;
                } else {
                    $startCounter = false;
                }
                ...
                foreach ($timevar as $key => $value) {
                    if ($outrow[$z][$nameKey] == $key) {
                        $outrow[$z][$value] = $outrow[$z][$timeKey];
                    } else {
                        $outrow[$z][$value] = 0;
                    }
                }
    ...
    if ($act == "play") {// start of play part
        $expname = $_POST['expname'];
...
//generate procdata string
        ...
        <form>
            <table>
                <TR>
                    <TD><P>Subject selection</P><select name="subj" onChange="showData(this.form)" class="tdstyle">
                    ...
                    <TD><P>Process data of current subject</P><textarea name="proctxt" cols=80 rows=10
                                                                        class="tdstyle"></textarea></TD>
                </TR>
            </table>
            <input type="button" onClick="playData()" value="Playback Data">
            <p><strong>Note:</strong>The Replay routine assumes that the php-file you want to replay has the same name
                as the experiment itself: <?php echo($expname); ?>.php . You can change the name of the file on the
                replay page if it is different from the file name.</p>
        </form>
        </body>
        </HTML>
        <?php
...
// startup code for making a selection (if form is not submitted, first opening)
    ...
    <form method="post" action="datalyser.php">
        ...
        <P>
            ...
            This is a textfile format in which each field is enclosed in brackets (") and separated by commas. Such a
            file can be read by most statistical programs. If the <strong>unpack events</strong> box is checked, the
            program will unpack the process data (whether it is in XML or CSV format in the database) into a list of
            events.
        </P>
        <p>The <strong>download and process selected</strong> button allows to download processed data that can be
            analyzed directly. It will delete acquisitions below the threshold, will calculate time and frequency
            columns for each box on the screen, and will summarize data in divisions.</p>
        <p><strong>Disclaimer: The processing module has not been checked extensively for the 1.00beta version! Check
                whether the output is consistent with the event files.</strong></p>
        <p>
            The <strong>Show Table</strong> button allows you to look at the data in one table, either unpacked or as
            is. The <strong>Playback</strong> allows for playback of participants in one of the experiments. This button
            wil open a new page in which you can select a participant from the list.</P>
        <P>
            <strong>Password:</strong> For any action you do on this page, a password is required. Type the password
            before pressing a button. This prevents unauthorized users that browse to this page from actually reading
            your data!
        </P>
        <Table border=1>
            <TR>
            ...
            while ($rec = mysql_fetch_row($result)) {
                ...
                # Execute SQL query
                $res2 = mysql_query($sqlQuery) or die("Invalid Query : " . mysql_error());
                while ($rec2 = mysql_fetch_row($res2)) {
                    if ($rec2[0] != "") {
                        $playstr = "<input type=button value=\"Replay\" onClick=\"replay('$rec[0]')\">";
                        break;
                    }
                }
            ...
            <TR>
                <TD>&nbsp;</TD>
                <td><input type=button value="sel all" onClick="selecting(1);"> <input type=button value="Reset sel"
                                                                                       onClick="selecting(2);"><br><input
                            type=button value="Invert sel" onClick="selecting(3)"></td>
                <TD>&nbsp;</TD>
                <TD>&nbsp;</TD>
            </TR>
            <TR>
            ...
            <tr>
                <TD colspan=3><input type=button value="download and process selected" onClick="process()"><br/>Threshold
                    (ms)<input type=text name=threshold value='200'><br/>divisions (1=all, 2=halfs ect.):<input
                            type=text name=divisions value='1'></TD>
            </TR>
        </Table>
    </form>
